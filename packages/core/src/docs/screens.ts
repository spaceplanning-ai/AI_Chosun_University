...
const ADMIN_SCREENS: readonly DocScreen[] = [
  ...
  },
  {
    ...
    requirement:
      '현장에서 받은 설문 응답을 옮겨 적는다. 문항 구성은 연구 모형이 설문으로 재도록 정한 다섯 가지를 그대로 따른다. 설문으로만 얻을 수 있는 값이므로 시스템이 만들어 내지 않으며, 입력 전에는 목록이 비어 있다.',
    ...
    areas: [
      { name: '목록', content: '응답 식별자와 항목별 점수.', source: '관리자 입력' },
      {
        name: '등록·수정 창',
        content:
          '인지된 적합성·설명 이해도·인지된 신뢰도·만족도·방문 의도 다섯 문항(각 1~7점)과 동행 유형·자유 응답.',
        source: 'apps/admin/src/config/resourceSchemas.ts',
      },
    ],
    features: [
